...
(function() {
	...
	NAMES[0x8769][0xA005] = NAMES; /* exif interopindex -> tiff ifd :) */
	...
	Mapy.EXIF = class {
		...
		_readEXIF(start, length) {
			...
			this._readIFD(tiffStart+firstOffset, tiffStart, NAMES);
		}

		_readIFD(ifdStart, tiffStart, names) {
		...
		_readTagValue(tagStart, tiffStart) {
		...
		_setBigEndian(bigEndian) {
			this._bigEndian = bigEndian;
		}

		_getValue(index, length) {
			var len = length || 1;
			var result = 0;
